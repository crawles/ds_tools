...
def _apply_to_struct(fxn, return_type, struct_col):
    """
...
def apply_py_fxn(fxn, return_type, *cols):
    struct_args = [F.col(c) for c in cols]
    struct = F.struct(*struct_args)
    struct_list = F.collect_list(struct)
    return _apply_to_struct(fxn, return_type, struct_list).alias(fxn.__name__)
